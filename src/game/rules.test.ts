...
describe("resolveSimultaneous — 1.5手の権利（ルール②・星取得と追加ポア）", () => {
  ...
  it("消費(白・対称): white=1.5 で 白主手(7,7)＋追加ポア(8,8) → (8,8)=白0.5・moveRights.white===0", () => {
    const state = createInitialState("9");
    state.moveRights.white = 1.5;
    const r = resolveSimultaneous(
      def,
      state,
      null, // 黒はパス
      { type: "place", x: 7, y: 7, placeKind: "stone" }, // 白の主手
      null,
      { x: 8, y: 8 }, // whiteExtra（追加ポア・非星の空点）
    );
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.state.cells[indexOf(def, 8, 8)]).toBe(-0.5); // 追加ポアで白0.5
    expect(r.state.moveRights.white).toBe(0); // 追加ポアを使ったら消費 → 0（非星なので取得なし）
  });

  it("星から動き出す move は非取得: 黒0.5を星(2,2)→非星(1,1)へ move → 着地点判定で権利は増えない（from は無関係）", () => {
    const state = createInitialState("9");
    state.cells[indexOf(def, 2, 2)] = 0.5; // 星(2,2)に黒0.5
    const r = resolveSimultaneous(
      def,
      state,
      { type: "move", fromX: 2, fromY: 2, toX: 1, toY: 1 }, // 星から非星へ動く
      null,
    );
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.state.cells[indexOf(def, 1, 1)]).toBe(0.5); // 非星へ 0.5 が乗る
    expect(r.state.cells[indexOf(def, 2, 2)]).toBe(0); // 星の元は空く
    expect(r.state.moveRights.black).toBe(0); // 着地点が非星なので取得しない（from が星でも無関係）
  });

  it("extra が自分の main(1石)と同点で超過: black=1.5・place stone(3,3)＋追加ポア(3,3) → {ok:false, which:black, reason:illegal-landing}・state 不変", () => {
    const state = createInitialState("9");
    state.moveRights.black = 1.5;
    const cellsBefore = [...state.cells];
    const r = resolveSimultaneous(
      def,
      state,
      { type: "place", x: 3, y: 3, placeKind: "stone" }, // 主手 1石 (+1)
      null,
      { x: 3, y: 3 }, // 追加ポア (+0.5) を同点へ → 合算 1.5 は値域外
    );
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.which).toBe("black"); // 白でなく黒起因で正しく報告
    expect(r.reason).toBe("illegal-landing");
    expect([...state.cells]).toEqual(cellsBefore); // 元 state 不変（resolveAdd throw に落ちない）
    expect(state.turnCount).toBe(0);
  });

  it("extra が自分の main(ポア)と同点で合法(1.0): black=1.5・place pour(3,3)＋追加ポア(3,3) → ok・cells[(3,3)]===1（ポア+ポア=1石相当）", () => {
    const state = createInitialState("9");
    state.moveRights.black = 1.5;
    const r = resolveSimultaneous(
      def,
      state,
      { type: "place", x: 3, y: 3, placeKind: "pour" }, // 主手 ポア (+0.5)
      null,
      { x: 3, y: 3 }, // 追加ポア (+0.5) を同点へ → 合算 1.0 は正当な自石生成
    );
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.state.cells[indexOf(def, 3, 3)]).toBe(1); // 0.5 + 0.5 = 黒1石
    expect(r.state.moveRights.black).toBe(0); // 追加ポアを使ったので消費 → 0
  });
});
